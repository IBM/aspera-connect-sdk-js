  ...
  stages {
    stage('Copy Installers') {
      steps {
        ...
        copyArtifacts filter: 'installer/BUILD/linux-g2.12-64-debug/ibm-aspera-connect*.tar.gz', fingerprintArtifacts: true, flatten: true, projectName: 'apps-trunk-build-linux-64', target: 'imports/dist/sdk'
        sh 'env | sort'
      }
    }
    stage('Build - SDK') {
