...
class NativeMessageExtRequestImplementation extends ExtRequestImpl {
  ...
  init = (options: IReqInitOptions) => {
    ...
    let disconnect = (evt: any) => {
      ...
      if (evt.detail) {
          var installIssueDetected = false;
          [
      ...
      if (data) {
          ...
          if ('body64' in data) {
              ...
              if (data.complete == true) {
                  var resp = this.atou(this.outstandingRequests[id].response);
...
var _hasConnectBeenInstalled = function _hasConnectBeenInstalled() {
    var detected = window.localStorage.getItem(ExtRequestImpl.LS_CONNECT_DETECTED);
    return detected && detected != '';
};

var _recordConnectInstall = function _recordConnectInstall() {
