...
let encryptRequired = function (sessionId: string | null | undefined) {
  return sessionId !== undefined && sessionId !== null;
};

let encryptReqBody = function (data: any, sessionId: string | null | undefined) {
  let dataToSend = data;
...
class XMLhttpRequestImplementation {
  ...
  enterWaitReady = () => {
    let waitReady = () => {
      ...
      let method = 'GET';
      let path = '/connect/info/ready';
      let requestInfo = { id: requestId, method: method, port: this.connectPort, path: path, data: null, callbacks: null };
  ...
  versionCheck = () => {
      // Minimum compatibility check (3.8+)
    let currentRequestId = this.nextId++;
  ...
  iteratePortsCallback = (httpCode: number, response: any, requestId: string) => {
    ...
    let checkedPort = this.idCallbackHash[requestId].port;
    delete this.idCallbackHash[requestId];
    ...
    if (checkedPort === DEFAULT_PORT) {
      ...
      for (let port = DEFAULT_PORT + 1; port < (DEFAULT_PORT + MAX_PORT_SEARCH); port++) {
        let currentRequestId = this.nextId++;
        let method = 'GET';
        let path = '/connect/info/ping';
        let requestInfo = { id: currentRequestId, method: method, port: port, path: path, data: null, callbacks: null };
  ...
  iteratePorts = (firstRun = false) => {
    ...
    let requestInfo = { id: requestId, method: method, port: DEFAULT_PORT, path: path, data: null, callbacks: null };
    this.idCallbackHash[requestId] = requestInfo;
  ...
  fastInit = () => {
    ...
    let id = sessionStorage.getItem(SS_SESSION_LASTKNOWN_ID);
    ...
    let key = sessionStorage.getItem(SESSION_LASTKNOWN_KEY);
    ...
    let port = sessionStorage.getItem(SESSION_LASTKNOWN_PORT);
  ...
  httpRequest = (method: string, path: string, data: any, callback: any, requestId: number, sessionId: string | null) => {
    ...
    let fullpath = LOCALHOST + this.connectPort + version + path;
