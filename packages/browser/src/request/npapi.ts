...
class NPAPIrequestImplementation {
  ...
  createNPAPIPlugin = (initializeTimeout: number) => {
      var wrapperDiv = document.getElementById(this.listenerId);
      if (wrapperDiv == null) {
      ...
      this.npapiPlugin = <IConnectPlugin>document.createElement('object');
  ...
  init = (options: INPAPIOptions) => {
      ...
      var changeConnectStatus = options.requestStatusCallback || function(){};
      try {
          if (!this.isSupportedByBrowser()) {
                  ...
                  mimeType() in navigator.mimeTypes)
              {
                   ...
                   */
                  var npapiWaitPluginLoadedID = setInterval(() => {
      ...
      httpRequest = (method: string, path: string, data: any, callback: any, requestId: string) => {
          if (this.npapiPlugin == null)
              return;
          var requestCallback = function(data: any) {
